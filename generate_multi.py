...
class CheckpointSelector:
    ...
    def __init__(self, version: str, base_path: str = "./results/lightning_logs"):
        ...
        self.checkpoint_dir = os.path.join(base_path, f"version_{version}", "checkpoints")
    ...
    def find_latest_n(self, n: int) -> List[str]:
        ...
        checkpoints_with_time = [(ckpt, os.path.getmtime(ckpt)) for ckpt in all_checkpoints]
    ...
    def find_periodic_only(self) -> List[str]:
        ...
        for ckpt in all_checkpoints:
            ...
            if any(pattern in filename for pattern in ['best_val_loss', 'best_loss', 'last.ckpt']):
                continue
...
def generate_output_filename(checkpoint_path: str, version: str, output_dir: str) -> str:
...
def main():
    ...
    parser.add_argument("--version", type=str, required=True, help="Model version number")
    parser.add_argument("--base_path", type=str, default="./results/lightning_logs", 
    ...
    selection_group.add_argument(
        "--epoch_range", 
        type=str, 
        help="Epoch range (e.g., '50-100')"
    )
    selection_group.add_argument(
        "--step_range", 
        type=str, 
        help="Step range (e.g., '1000-5000')"
    )
    selection_group.add_argument(
        "--pattern", 
        type=str, 
        help="Glob pattern for checkpoint names (e.g., '*best*')"
    )
    selection_group.add_argument(
        "--latest_n", 
        type=int, 
        help="Use the N most recent checkpoints"
    )
    selection_group.add_argument(
        "--periodic_only", 
        action="store_true", 
    ...
    parser.add_argument("--output_dir", type=str, help="Output directory (default: ./generated_samples/version_X)")
    parser.add_argument("--num_samples", type=int, default=32, help="Number of samples to generate")
    parser.add_argument("--condition_file", type=str, help="Path to condition file")
    parser.add_argument("--noise_file", type=str, help="Path to predetermined noise file")
    parser.add_argument("--batch_size_per_gpu", type=int, default=12, help="Batch size per GPU")
    parser.add_argument("--img_size", type=int, default=96, help="Image size")
    parser.add_argument("--channels", type=int, default=1, help="Number of channels")
    parser.add_argument("--inf_timesteps", type=int,default=100,help="Inference timesteps")
    ...
    try:
        ...
        for ckpt_idx, checkpoint in enumerate(checkpoints, 1):
            ...
            output_path = generate_output_filename(checkpoint, args.version, args.output_dir)
            
            # Check if output already exists
            if os.path.exists(output_path):
                print(f"⏭️  Output already exists, skipping: {os.path.basename(output_path)}")
                success_count += 1
                continue
